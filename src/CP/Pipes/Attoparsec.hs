...
parse' parser = S.StateT $ \p0 -> do
    ...
    case x of
      Left r       ->
        case feed (_parse parser mempty) mempty of
          Fail _ c m -> return (Left (ParsingError c m), return r)
          Done _ b   -> return (Right b, return r)
  ...
  where
    step diffP res p0 = case res of
      ...
      Done a b   -> return (Right b, yield a >> p0)
      Partial k  -> do
        x <- nextSkipEmpty p0
        case x of
...
parseThrow
  :: (MonadThrow m, ParserInput a)
  => Attoparsec.Parser a b
  -> Pipes.Parser a m b
parseThrow = fromRightMM . parse'
